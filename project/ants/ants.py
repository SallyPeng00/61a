...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        moved_dist = 0 
        while place != hive:
            if len(place.bees)> 0 and self.min_range <= moved_dist <= self.max_range:
...
def make_scare(action, bee):
    ...
    """
    # BEGIN Problem EC
    def scare(colony, direction=-1):
        Bee.bee_scared.append(bee)
        return action(colony, direction)
    return scare
    # END Problem EC
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target not in Bee.bee_scared:
