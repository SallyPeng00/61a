...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                self.ant = insect
            elif self.ant.can_contain(insect) and self.ant.contained_ant is None:
...
add_insect        remove_insect tries to remove an Ant that is not anywhere in this
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.scared != 0 and backward != None:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        i = 0 
        while place != hive:
            if len(place.bees)> 0 and self.min_range <= i <= self.max_range:
        ...
        if not self.true_queen:
            ...
            while place.exit != None:
                if place.ant is not None:
                    ...
                    if place.ant.is_container and (place.ant.contained_ant is not None) and not place.ant.contained_ant.buffed:
...
def apply_effect(effect, bee, duration):
    ...
    def counter(action):
        def counted(*args):
            ...
            if counter.count >= duration:
                bee.action = old_action
                print(("DEBUG: old_action"))

            action(*args)
        counter.count = 0
        return counted

    bee.action = counter(effect(bee.action, bee))
