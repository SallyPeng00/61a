...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        function_form = do_lambda_form(Pair(target.second, expressions.second), env)
        env.bindings[target.first] = function_form
        return target.first
        # END PROBLEM 10
    else:
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if clause.second is nil:
                # print("DEBUG:"scheme_eval(clause.first, env))
                return test
            else:
...
def make_let_frame(bindings, env):
    ...
    while bindings_copy is not nil:
        check_form(bindings_copy.first, 2, 2)
        bindings_copy = bindings_copy.second

    # print("DEBUG: done with check_form")
    ...
    formals, vals = get_formals(bindings), get_vals(bindings)
    # print('DEBUG: DONE WITH FORMAL VAL EVALUATION', formals, vals)
    check_formals(formals)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(expressions.first, Pair):
        ...
        # print("DEBUG:", env.bindings[expressions.first.first])
        # print("DEBUG:", expressions.first.first)
