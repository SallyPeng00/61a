...
def group_by_centroid(restaurants, centroids):
    ...
    #     groups += [[find_closest(restaurant_location(r), centroids), r]]
    # return group_by_first(groups)

    return group_by_first([find_closest(restaurant_location(r), centroids) for r in restaurants])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictor_lst = [find_predictor(user, reviewed, feature) for feature in feature_fns]
    best_predictor, r_squared = max(predictor_lst, key=lambda x: x[1])
